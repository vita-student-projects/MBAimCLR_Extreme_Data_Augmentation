...
from net.att_drop import Simam_Drop###ADD
...
class Transformer(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=256, decoder_dim_feat=256,
        ...
        # ADD
        self.head = nn.Linear(dim_feat, dim_out) if dim_out > 0 else nn.Identity()
        self.eadm = Simam_Drop(num_point=num_joints, keep_prob=0.7) 
    ...
    def forward_encoder(self, x, drop = False, N=32, M=2):
        ...
        x = x.reshape(NM, TP * VP, -1) #For passing into the blocks
        ...
        # apply the energy based drop if we need to
        if drop:
            x = x.view(NM,TP,VP,-1)
            x = x.permute(0,3,1,2)
            y = self.eadm(x)
        ...
        # change the order of the data to fit what order we have in AimCLR framework
        x = x.view(NM,TP,VP,-1)
        x = x.permute(0,3,1,2)
        x = F.avg_pool2d(x, x.size()[2:])
        x = x.view(N,M,-1).mean(dim=1)
    ...
    def forward(self, x, drop=False):
        ...
        # permute the data to be in the order of what the transformer model expect
        x = x.permute(0, 4, 2, 3, 1).contiguous().view(N * M, T, V, C)
        
        #we only use the encoder
        return self.forward_encoder(x, drop=drop, N=N, M=M)
